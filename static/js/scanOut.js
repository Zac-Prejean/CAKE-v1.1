...
async function displayItemInfo(scannedNumber) {  
    ...
    try {  
        ...
        const [_, order_number, itemName, , sku, batch] = scannedItem.split('_'); // Extract sku and batch  
        ...
        // Update the printed count for the current user  
        const scanOutData = {  
        ...
        document.getElementById('confirmButton').addEventListener('click', async function () {  
            if (orderTotalQty == 1) {  
                showSingleModal("Are you ready to ship?", "CONFIRM", "CANCEL", item_id, itemName, order_number, async function () {  
                    ...
                    await fetch('/scan_out', {  
                        method: 'POST',  
                        headers: {  
                            'Content-Type': 'application/json',  
                        },  
                        body: JSON.stringify(scanOutData),  
                    });  
                }, async function () {  
                    await updateStatus(order_number, item_id, itemName, "Scanned-Out", filename);  
                    await fetch('/scan_out', {  
                ...
                if (currentStatusText.toUpperCase() === "PRINTED") {  
                    showQuantityModal("Adding to CUBBY", "CONFIRM", null, item_id, itemName, null, async function () {  
                        const boxNumber = await addToCubby(order_number, item_id, itemName, orderTotalQty, filename, sku, batch); // Pass sku and batch  
                        ...
                        setTimeout(() => {  
                            console.log("Showing Box Number Modal with cubbyID:", boxNumber);  
                            showBoxNumberModal(boxNumber); // Pass the correct box number (cubbyID)  
...
async function updateStatus(order_number, item_id, itemName, new_status, filename, cubby_number) {  
    ...
    formData.append('cubby_number', cubby_number); 
...
fetch('/get_scanned_numbers')  
    ...
    async function addToCubby(order_number, item_id, itemName, totalQuantity, filename, sku, batch) {  
        ...
        const existingBoxes = scannedNumbers.filter(numberWithDate => numberWithDate.orderID === order_number);  
      
        let nextEmptyBox;  
        // If there are no existing boxes for the current order_number, find the next available empty box  
        if (existingBoxes.length === 0) {  
            nextEmptyBox = 1;  
            while (scannedNumbers.some(numberWithDate => numberWithDate.cubbyID === nextEmptyBox)) {  
                nextEmptyBox++;  
            }  
        } else {  
            // If there are existing boxes for the current order_number, use the same box number  
            nextEmptyBox = existingBoxes[0].cubbyID;  
        }  
      
        const cubbyID = nextEmptyBox; // The box number is used as cubbyID  
        const dateTime = new Date().toLocaleString();  
      
        // Form data to send in the POST request  
        const formData = new URLSearchParams();  
        formData.append('itemID', item_id);  
        formData.append('orderID', order_number);  
        formData.append('qty', totalQuantity);  
        formData.append('sku', sku);  
        formData.append('cubbyID', cubbyID);  
        formData.append('batch', batch);  
        formData.append('dateTime', dateTime);  
        formData.append('scannedBy', signedInEmployeeName);  
        ...
        scannedNumbers = data.scanned_numbers;  
        return data.cubbyID; // Return the cubbyID from the server response  
    }    
...
function showModal(status) { 
  ...
  function showBoxNumberModal(cubbyID) {  
    const boxNumberModal = document.getElementById("boxNumberModal");  
    document.getElementById("boxNumber").innerText = cubbyID; // Ensure this is using cubbyID  
