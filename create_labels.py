...
def create_labels(index, row, folder_name, sku, clean_sku, order_quantities, order_skus, process_label, order_item_count, order_total_qty,  customtagID):  
    ...
    # "BLABEL" orders  
    if clean_sku.startswith("BLABEL") or clean_sku.startswith("GIL") or clean_sku.startswith("BEL") or clean_sku.startswith("HAN") or clean_sku.startswith("LAT"):
        png_path_front = None  
        png_path_back = None  
        ...
        if process_label:  
            ...
            designs = get_front_back(order_number, item_options)
            front_png_exists = designs[0]
            #print("Front exists?:",front_png_exists)
            back_png_exists = designs[1]
            #print("back exists?:",back_png_exists)
