...
def updateStatus(itemID, order_number, itemName, new_status, scanned, filename, cubby_number):    
    ...
    designed_file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), CAKE_PATH, 'Batch.txt', 'DTG', 'status', filename)    
    archive_file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), CAKE_PATH, 'Batch.txt', 'DTG', 'archive', f"{filename.split('.')[0]}_archive.txt")    
...
def updateAllItemsStatus(order_number, new_status, filename, cubby_number=None):    
    file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), CAKE_PATH, 'Batch.txt', 'DTG', 'status', filename)  
...
def get_txt_files(folder="status"):  
    if folder == "archive":  
        folder_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), CAKE_PATH, 'Batch.txt', 'DTG', 'archive')  
    else:  # default to "status" folder  
        folder_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), CAKE_PATH, 'Batch.txt', 'DTG', 'status')  
